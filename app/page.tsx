...
function LoginPage({ onLogin, allMembers }: { onLogin: (m: Member) => void, allMembers: Member[] }) {
  ...
  return (
    <div className="min-h-screen bg-[#F9F8F3] flex flex-col items-center justify-center p-8 text-center font-sans">
      ...
      <p className="text-sm text-gray-400 font-bold mb-12 uppercase tracking-widest tracking-tighter">多人雲端同步版</p>
  ...
  return (
    <div className="min-h-screen bg-[#F9F8F3] p-8 font-sans">
      ...
      <div className="flex justify-between items-center mb-6">
        <h3 className="text-sm font-black text-[#5E9E8E] uppercase tracking-tighter">我的旅遊清單</h3>
      ...
      {showAddMember && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-md z-[100] p-8 flex items-center justify-center">
          <div className="bg-white w-full max-w-md p-8 rounded-[48px] shadow-2xl">
            <h3 className="text-xl font-black mb-6 text-black italic">Member Setup</h3>
            <div className="flex flex-col items-center mb-6 gap-4">
               <div className="w-20 h-20 rounded-full bg-gray-100 overflow-hidden border-2 border-dashed border-gray-300">
                  {memberForm.avatar ? <img src={memberForm.avatar} className="w-full h-full object-cover" /> : <div className="flex items-center justify-center h-full text-xs text-gray-400 font-black">無照片</div>}
               </div>
            ...
            <input placeholder="登錄代碼 (如 wayne)..." value={memberForm.loginCode} onChange={e=>setMemberForm({...memberForm, loginCode: e.target.value})} className="w-full p-4 bg-gray-50 rounded-2xl mb-6 font-black text-black outline-none" />
            <div className="flex gap-4"><button onClick={() => setShowAddMember(false)} className="flex-1 py-4 bg-gray-100 text-gray-400 rounded-3xl font-black">關閉</button><button onClick={() => { setAllMembers([...allMembers, {id: Date.now().toString(), ...memberForm}]); setShowAddMember(false); }} className="flex-1 py-4 bg-blue-500 text-white rounded-3xl font-black shadow-lg">完成新增</button></div>
          </div>
        </div>
      )}
    </div>
...
function MainApp({ onBack, user, tripData }: { onBack: () => void, user: Member, tripData: Trip }) {
  ...
  const [payMethod, setPayMethod] = useState('現金'); // 修復點：新增 payMethod 狀態
    ...
    <div className="min-h-screen bg-[#F9F8F3] font-sans pb-32 text-black font-black">
      ...
      <div className="p-4 flex justify-between items-center sticky top-0 bg-[#F9F8F3]/90 backdrop-blur-md z-40">
        <div onClick={onBack} className="flex items-center gap-3 cursor-pointer group active:scale-95 transition-all">
          <div className="w-10 h-10 bg-white rounded-2xl flex items-center justify-center shadow-sm border border-gray-100 text-black text-xl group-hover:bg-[#5E9E8E] group-hover:text-white transition-all">←</div>
          <h1 className="text-xl font-black text-[#5E9E8E] tracking-tighter leading-none">{tripData.title}</h1>
        </div>
      ...
      {activeTab === '行程' && (
        <div className="px-4">
          <div className="bg-[#5E9E8E] rounded-[32px] p-6 text-white mb-6 shadow-lg">
            ...
            <p className="text-sm font-bold uppercase tracking-widest">札幌地區 · DAY {activeDay}</p>
          </div>
          ...
          <div className="mt-6 relative">
            ...
            {(schedules[activeDay] || []).sort((a,b)=>a.time.localeCompare(b.time)).map(item => (
              <div key={item.id} className="flex gap-4 mb-8 relative group">
                ...
                <div className="flex-1 bg-white p-5 rounded-[24px] shadow-sm border border-orange-50 relative min-h-[100px]">
                  ...
                  <p className="text-[10px] text-gray-500 leading-relaxed mb-6 whitespace-pre-wrap">{item.desc}</p>
                  <div className="absolute bottom-3 right-4 flex gap-4 text-gray-300">
                    <button onClick={() => window.open(`https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(item.title)}`)} className="text-[#5E9E8E] text-[10px] font-black italic tracking-tighter">NAVIGATE</button>
            ...
            <button onClick={() => setShowPlanModal({show:true, type:'add'})} className="ml-14 w-[calc(100%-3.5rem)] py-4 border-2 border-dashed border-gray-200 rounded-2xl text-gray-400 text-sm font-bold bg-white/50 tracking-tighter">+ ADD NEW STOP</button>
          </div>
        </div>
      )}
        ...
        <div className="px-4">
          ...
          {bookSubTab === '機票' && (
            <div className="space-y-10">
              <button onClick={() => setShowFlightModal({show:true, type:'add'})} className="w-full py-4 border-2 border-dashed border-gray-200 rounded-[32px] text-gray-400 font-black text-xs tracking-widest uppercase">+ ADD FLIGHT INFO</button>
              {flights.map(f => (
                <div key={f.id} className="bg-white rounded-[40px] shadow-2xl overflow-hidden border border-blue-50 relative">
                  <div className="bg-[#f8faff] p-8 pb-10 border-b border-dashed border-blue-100 relative text-center">
                    ...
                    <h2 className="text-6xl text-black font-black tracking-tighter">{f.flightNo}</h2>
                  ...
                  <div className="p-8 pt-12 text-center font-black">
                    ...
                    <div className="grid grid-cols-2 gap-4 border-t border-dashed pt-8 border-gray-100"><div className="bg-[#f0f4ff]/50 p-4 rounded-[24px] text-center font-black text-black shadow-inner"><p className="text-[9px] text-blue-300 uppercase">🧳 BAGGAGE: {f.baggage}</p></div><div className="bg-[#f0f4ff]/50 p-4 rounded-[24px] text-center font-black text-black shadow-inner"><p className="text-[9px] text-blue-300 uppercase">✈️ {f.aircraft}</p></div></div>
                  </div>
                </div>
              ))}
            </div>
          ...
          {bookSubTab === '憑證' && (
            <div className="space-y-6">
              <button onClick={() => { 
                ...
                if(title) { 
                  // 修正點：使用 string 轉型避免型別不相容
                  const n = [{id: Date.now(), type:'憑證' as string, title, content:'', image: prompt("照片網址:") || undefined}, ...bookings]; 
                  setBookings(n); syncToCloud({bookings:n}); 
                } 
              }} className="w-full py-4 border-2 border-dashed border-gray-200 rounded-[32px] text-gray-400 font-black text-xs uppercase tracking-widest">+ ADD VOUCHER</button>
              {bookings.map(b => (
      ...
      {activeTab === '記帳' && (
        <div className="px-4">
          ...
          <div className="bg-[#E9C46A] rounded-[24px] p-6 mb-6 text-white shadow-md font-black tracking-tighter"><p className="text-sm opacity-90 italic uppercase">TOTAL EXPENSES (TWD)</p><h2 className="text-4xl font-mono">NT$ {records.reduce((sum, r) => sum + Number(r.twdAmount), 0).toLocaleString()}</h2></div>
          <div className="bg-white rounded-[32px] p-6 shadow-sm border border-orange-50 mb-8 relative text-black">
            <input value={category} onChange={e => setCategory(e.target.value)} placeholder="消費項目 (例: 豪華蟹肉餐)..." className="w-full p-4 bg-[#F2F1EB] rounded-2xl mb-4 outline-none font-black" />
            <div className="grid grid-cols-2 gap-4 mb-6 font-mono font-black text-2xl tracking-tighter overflow-hidden">
              ...
              <div className="w-full p-4 bg-[#F2F1EB] rounded-2xl text-gray-300 flex items-center tracking-tighter">≈ {(Number(amount)*JPY_TO_TWD).toFixed(0)}</div>
            </div>
            {/* 修復點：使用 setPayMethod 狀態 */}
            ...
            }} className="w-full py-4 bg-[#86A760] text-white rounded-2xl font-black shadow-lg uppercase tracking-widest">SAVE EXPENSE</button>
          </div>
          <div className="space-y-3 pb-10">
             {records.map(r => (
               <div key={r.id} className="bg-white p-5 rounded-2xl flex justify-between items-center shadow-sm relative pr-20 border border-gray-50 font-black text-black">
                 <div className="text-xs">{r.category} ({r.payMethod})<p className="opacity-50 font-normal font-mono text-[10px] mt-1 italic">{r.payer} · {r.date}</p></div>
                 <div className="text-right text-[#5E9E8E] font-mono tracking-tighter">{r.amount} JPY</div>
                 <div className="absolute right-3 flex flex-col gap-2"><button onClick={() => { if(confirm('確定刪除紀錄？')) { const n = records.filter(i => i.id !== r.id); setRecords(n); syncToCloud({records: n}); } }} className="text-xs opacity-20 hover:opacity-100">🗑️</button></div>
               </div>
             ))}
          </div>
        </div>
      ...
      {activeTab === '日誌' && (
        <div className="px-4">
          <div className="bg-white rounded-[32px] p-6 shadow-sm border border-orange-50 mb-8 relative text-black">
            <h3 className="text-sm font-black mb-4 text-[#5E9E8E] uppercase tracking-tighter italic tracking-widest">{newJournal.id ? 'EDITING JOURNAL' : 'TODAY\'S JOURNAL'}</h3>
            <textarea value={newJournal.content} onChange={e => setNewJournal({...newJournal, content: e.target.value})} placeholder="記錄下此刻的心情..." className="w-full p-4 bg-gray-50 rounded-2xl mb-4 outline-none text-sm h-32 leading-relaxed font-black" />
            <div className="flex justify-between items-center">
              <button onClick={() => { const u = prompt("照片網址:"); if(u) setNewJournal({...newJournal, image: u}); }} className="text-[10px] bg-gray-100 px-3 py-2 rounded-xl font-black">📷 ADD PHOTO</button>
              <button onClick={() => {
                if(!newJournal.content) return;
                let n;
                if(newJournal.id) {
                   n = journals.map(j => j.id === newJournal.id ? {...j, content: newJournal.content, image: newJournal.image} : j);
                } else {
                   n = [{ id: Date.now(), author: user.name, content: newJournal.content, date: "2026/01/10", image: newJournal.image }, ...journals];
                }
                setJournals(n); syncToCloud({journals: n});
                setNewJournal({id: null, content:'', image:''});
              }} className="px-8 py-3 bg-[#86A760] text-white rounded-2xl font-black shadow-lg italic uppercase tracking-widest">PUBLISH</button>
            </div>
          </div>
          {journals.map(j => (
            <div key={j.id} className="bg-white p-8 rounded-[32px] shadow-sm mb-6 border border-gray-50 font-black text-black relative">
              <div className="flex items-center gap-3 mb-4 text-xs text-gray-300 italic tracking-tighter">
                ...
                <span>{j.author} · {j.date}</span>
                <div className="ml-auto flex gap-4 text-gray-300">
              ...
              {j.image && <img src={j.image} className="w-full rounded-[24px] shadow-lg mt-4 border border-gray-100" />}
            </div>
          ))}
        </div>
      )}
      ...
      {activeTab === '準備' && (
        <div className="px-4">
          ...
          <div className="bg-white rounded-[32px] p-6 shadow-sm mb-8 border relative">
            <input value={newTodoInput.task} onChange={e => setNewTodoInput({...newTodoInput, task: e.target.value})} placeholder={`輸入${prepSubTab}事項...`} className="w-full p-4 bg-gray-50 rounded-2xl mb-4 outline-none font-black text-black" />
            <div className="flex gap-2 mb-6 overflow-x-auto no-scrollbar">
                {tripData.members.map(m => (
                  <button key={m.id} onClick={() => { const ids = newTodoInput.assigneeIds.includes(m.id) ? newTodoInput.assigneeIds.filter(i => i !== m.id) : [...newTodoInput.assigneeIds, m.id]; setNewTodoInput({...newTodoInput, assigneeIds: ids}); }} className={`p-2 px-3 rounded-xl border text-[10px] font-black transition-all ${newTodoInput.assigneeIds.includes(m.id) ? 'bg-green-50 border-green-200 text-[#86A760] shadow-inner' : 'border-gray-50 text-gray-300'}`}>{m.name}</button>
                ))}
            </div>
            <button onClick={() => {
              if(!newTodoInput.task || newTodoInput.assigneeIds.length === 0) return alert("請輸入任務並指派成員");
          ...
          <div className="space-y-4">
            {todos.filter(t => t.category === prepSubTab).map(todo => (
              <div key={todo.id} className="bg-white p-6 rounded-[28px] shadow-sm border border-gray-100 relative font-black text-black">
                ...
                <div className="absolute top-6 right-6 flex gap-4 text-gray-200"><button onClick={() => { if(confirm('確定移除此項任務？')) { const n = todos.filter(t => t.id !== todo.id); setTodos(n); syncToCloud({todos: n}); } }} className="text-xs hover:text-red-500">🗑️</button></div>
              </div>
            ))}
          </div>
        </div>
      ...
      {activeTab === '成員' && (
        <div className="px-4 text-center">
          <h2 className="text-xl font-black mb-12 text-[#5E9E8E] uppercase italic tracking-tighter">Travel Squad</h2>
      ...
      {showPlanModal.show && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-[100] flex items-end font-sans">
          <div className="bg-white w-full p-8 rounded-t-[48px] shadow-2xl animate-in slide-in-from-bottom">
            ...
            <input placeholder="要去的地點..." value={planForm.title} onChange={e => setPlanForm({...planForm, title: e.target.value})} className="w-full p-5 bg-gray-50 rounded-[28px] mb-4 outline-none font-black text-black text-xl tracking-tighter placeholder:text-gray-300" />
            <textarea placeholder="細節備註..." value={planForm.desc} onChange={e => setPlanForm({...planForm, desc: e.target.value})} className="w-full p-5 bg-gray-50 rounded-[28px] mb-8 outline-none text-sm font-black text-black h-32 leading-relaxed tracking-tighter" />
            ...
            }} className="flex-1 py-4 bg-[#86A760] text-white rounded-3xl font-black shadow-xl uppercase italic">SAVE DESTINATION</button></div>
          </div>
        </div>
      )}
      ...
      {showFlightModal.show && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-[100] flex items-end font-sans">
          <div className="bg-white w-full p-8 rounded-t-[48px] shadow-2xl overflow-y-auto max-h-[90vh] text-black">
            <h3 className="text-2xl font-black mb-8 text-blue-500 uppercase italic tracking-tighter">FLIGHT DATA SETUP</h3>
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-2 gap-4 font-black"><div className="bg-gray-50 p-4 rounded-2xl tracking-tighter"><label className="text-[9px] text-gray-300 block mb-1 uppercase tracking-widest">Departure Code</label><input placeholder="TPE" value={flightForm.fromCode} onChange={e=>setFlightForm({...flightForm, fromCode:e.target.value})} className="w-full bg-transparent text-2xl outline-none text-black font-black" /></div><div className="bg-gray-50 p-4 rounded-2xl tracking-tighter"><label className="text-[9px] text-gray-300 block mb-1 uppercase tracking-widest">Arrival Code</label><input placeholder="CTS" value={flightForm.toCode} onChange={e=>setFlightForm({...flightForm, toCode:e.target.value})} className="w-full bg-transparent text-2xl outline-none text-black font-black" /></div></div>
              ...
              }} className="flex-1 py-4 bg-blue-500 text-white rounded-3xl font-black shadow-xl tracking-widest italic">CONFIRM FLIGHT</button></div>
            </div>
          </div>
        </div>
      )}
...
export default function AppEntry() {
  ...
  useEffect(() => {
    const savedMembers = localStorage.getItem('trip_members_v27');
  ...
  useEffect(() => {
    localStorage.setItem('trip_members_v27', JSON.stringify(allMembers));
  }, [allMembers]);
