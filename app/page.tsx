...
interface Flight { id: number; airline: string; flightNo: string; fromCode: string; toCode: string; depTime: string; arrTime: string; date: string; baggage: string; aircraft: string; }
  ...
  return (
    <div className="min-h-screen bg-[#F9F8F3] p-8 font-sans pb-32 text-black font-black">
      ...
      <div className="flex justify-between items-center mb-6">
        ...
        {user.loginCode === 'wayne' && <button onClick={() => setShowAddTrip(true)} className="text-[10px] bg-blue-500 text-white px-3 py-1 rounded-full font-black">＋ NEW TRIP</button>}
      </div>
      <div className="space-y-6">
        {allTrips.map(trip => (
          <div key={trip.id} className="relative group">
            ...
            {user.loginCode === 'wayne' && <button onClick={() => { if(confirm('確定刪除整趟旅程？')) onDeleteTrip(trip.id); }} className="absolute -top-2 -right-2 w-8 h-8 bg-red-500 text-white rounded-full text-xs shadow-lg">✕</button>}
          </div>
        ))}
      </div>
      {showAddTrip && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-md z-[100] p-8 flex items-center justify-center overflow-y-auto">
          <div className="bg-white w-full max-w-md p-8 rounded-[40px] shadow-2xl text-black font-black my-auto">
            ...
            <input value={newTrip.title} onChange={e=>setNewTrip({...newTrip, title:e.target.value})} className="w-full p-4 bg-gray-50 rounded-2xl mb-4 outline-none" />
            <div className="grid grid-cols-2 gap-4 mb-4">
              <div><label className="text-[10px] text-gray-400">START</label><input type="date" value={newTrip.startDate} onChange={e=>setNewTrip({...newTrip, startDate:e.target.value})} className="w-full p-3 bg-gray-50 rounded-xl text-xs" /></div>
              <div><label className="text-[10px] text-gray-400">END</label><input type="date" value={newTrip.endDate} onChange={e=>setNewTrip({...newTrip, endDate:e.target.value})} className="w-full p-3 bg-gray-50 rounded-xl text-xs" /></div>
            </div>
            <label className="text-[10px] text-gray-400 block mb-2">JOINING MEMBERS</label>
            <div className="flex flex-wrap gap-2 mb-8">
               {allMembers.map(m => (
                 ...
                 }} className={`px-4 py-2 rounded-full text-xs ${newTrip.memberIds.includes(m.id) ? 'bg-[#5E9E8E] text-white' : 'bg-gray-100 text-gray-400'}`}>{m.name}</button>
               ))}
            </div>
            <div className="flex gap-4">
              <button onClick={() => setShowAddTrip(false)} className="flex-1 py-4 bg-gray-100 rounded-3xl">Cancel</button>
              <button onClick={() => { onAddTrip({...newTrip, id: Date.now().toString()}); setShowAddTrip(false); }} className="flex-1 py-4 bg-[#86A760] text-white rounded-3xl shadow-lg">Save</button>
            </div>
          </div>
        </div>
      )}
...
function MainApp({ onBack, user, tripData, allMembers, onUpdateMembers }: { onBack: () => void, user: Member, tripData: Trip, allMembers: Member[], onUpdateMembers: any }) {
  ...
  const [flightForm, setFlightForm] = useState<Flight>({ id: 0, airline: 'EVA AIR', flightNo: 'BR116', fromCode: 'TPE', toCode: 'CTS', depTime: '09:30', arrTime: '14:20', date: '2026-01-10', baggage: '23kg', aircraft: 'B787' });
    ...
    <div className="min-h-screen bg-[#F9F8F3] font-sans pb-32 text-black font-black">
      ...
      <div className="p-4 flex justify-between items-center sticky top-0 bg-[#F9F8F3]/90 backdrop-blur-md z-40 text-black">
        <div onClick={onBack} className="flex items-center gap-3 cursor-pointer">
          ...
          <h1 className="text-xl font-black text-[#5E9E8E] tracking-tighter uppercase italic">DUPI TRAVEL</h1>
        </div>
      ...
      {activeTab === '行程' && (
        <div className="px-4 animate-in fade-in slide-in-from-bottom duration-500">
          <div className="bg-[#5E9E8E] rounded-[32px] p-6 text-white mb-6 shadow-lg">
            <h2 className="text-4xl font-black font-mono tracking-tighter">-5°C</h2>
            <p className="text-sm font-bold uppercase tracking-widest text-black">Day {activeDay} · {tripDates[activeDay-1]}</p>
          </div>
          <div className="flex gap-4 overflow-x-auto pb-4 no-scrollbar">
            {[1, 2, 3, 4, 5, 6, 7, 8].map(d => (
              <button key={d} onClick={() => setActiveDay(d)} className={`flex-shrink-0 w-14 h-20 rounded-2xl flex flex-col items-center justify-center transition-all ${activeDay === d ? 'bg-[#E9C46A] text-white shadow-lg scale-105' : 'bg-white text-gray-400 border'}`}>
                <span className="text-[10px] font-black text-black">{tripDates[d-1]}</span>
          ...
          <div className="mt-6 relative">
            ...
            {(schedules[activeDay] || []).sort((a,b)=>a.time.localeCompare(b.time)).map(item => (
              <div key={item.id} className="flex gap-4 mb-8 relative group">
                ...
                <div className="flex-1 bg-white p-5 rounded-[24px] shadow-sm border border-orange-50 min-h-[100px]">
                  ...
                  <p className="text-[10px] leading-relaxed opacity-60 mb-6 font-black">{item.desc}</p>
                  <div className="absolute bottom-3 right-4 flex gap-4">
                    <button onClick={() => window.open(`https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(item.title)}`)} className="text-[#5E9E8E] text-[10px] font-black italic uppercase tracking-tighter">MAP</button>
            ...
            {user.loginCode === 'wayne' && (<button onClick={() => setShowPlanModal({show:true, type:'add'})} className="ml-14 w-[calc(100%-3.5rem)] py-4 border-2 border-dashed border-gray-200 rounded-2xl text-gray-400 text-sm font-bold bg-white/50 tracking-tighter text-black">+ ADD DESTINATION</button>)}
          </div>
        </div>
      )}
      ...
      {activeTab === '預訂' && (
        <div className="px-4">
          <div className="flex bg-white rounded-full p-1 mb-6 shadow-sm border text-black font-black font-black">
            {['機票', '憑證'].map(t => (
          ...
          {bookSubTab === '機票' && (
            <div className="space-y-10">{flights.map(f => (
              <div key={f.id} className="bg-white rounded-[40px] shadow-2xl overflow-hidden border border-blue-50 relative">
                <div className="bg-[#f8faff] p-8 pb-10 border-b border-dashed border-blue-100 relative text-center text-black font-black font-black font-black">
                  <div className="flex justify-between items-center mb-6"><span className="bg-blue-600 text-white px-3 py-1 rounded-lg text-[11px] font-black uppercase">{f.airline}</span><button onClick={() => { if(confirm('要刪除嗎？')) { const n = flights.filter(i => i.id !== f.id); setFlights(n); syncToCloud({flights: n}); } }} className="text-xs opacity-20">🗑️</button></div>
                  <h2 className="text-6xl text-black font-black tracking-tighter uppercase italic">{f.flightNo}</h2>
                ...
                <div className="p-8 pt-12 text-center font-black text-black">
                  <div className="flex justify-between mb-10"><div><p className="text-4xl">{f.fromCode}</p><p className="text-blue-600 text-xl font-mono">{f.depTime}</p></div><div className="flex-1 px-4 flex flex-col items-center"><div className="w-full h-[2px] bg-blue-50 relative flex justify-center items-center"><span className="text-blue-400 text-xl transform rotate-90">✈️</span></div><p className="text-[10px] text-gray-400 mt-3">{f.date}</p></div><div><p className="text-4xl">{f.toCode}</p><p className="text-blue-600 text-xl font-mono">{f.arrTime}</p></div></div>
                </div>
              </div>
            ))}<button onClick={() => setShowFlightModal({show:true, type:'add'})} className="w-full py-4 border-2 border-dashed border-gray-200 rounded-[32px] text-gray-400 font-black tracking-widest uppercase text-black">+ Add Flight</button></div>
          )}
          {bookSubTab === '憑證' && (
            <div className="space-y-6">
              {bookings.map(b => (
                <div key={b.id} className="bg-white rounded-[32px] p-6 shadow-xl border border-gray-50 mb-4 text-black font-black">
              ...
              <div className="bg-white p-6 rounded-[32px] border-2 border-dashed border-gray-200 text-center font-black text-black">
                <p className="text-xs text-gray-300 mb-4">上傳電子憑證或截圖</p>
                <ImageUploader label="上傳憑證照片" onUpload={(b64) => {
                  const title = prompt("憑證名稱:"); if(title) { const n = [{id: Date.now(), type:'憑證', title, image: b64}, ...bookings]; setBookings(n); syncToCloud({bookings: n}); }
                }} />
              </div>
            </div>
          )}
      ...
      {/* 3. 記帳 (個人花費統計 + PayPay 編輯) */}
      {activeTab === '記帳' && (
        <div className="px-4">
          <div className="grid grid-cols-2 gap-3 mb-4 font-black text-black font-black">
            {allMembers.filter(m => tripData.memberIds.includes(m.id)).map(m => (
              <div key={m.id} className="bg-white p-4 rounded-3xl shadow-sm border border-[#E0F2F1]"><p className="text-[10px] text-black font-black uppercase italic">{m.name} PAID</p><p className="text-lg text-[#5E9E8E] font-mono tracking-tighter">NT$ {records.filter(r=>r.payerId===m.id).reduce((s,r)=>s+Number(r.twdAmount),0).toLocaleString()}</p></div>
            ))}
          </div>
          <div className="bg-[#E9C46A] rounded-[24px] p-6 mb-6 text-black shadow-md font-black italic"><p className="text-sm opacity-90 uppercase tracking-widest text-black font-black">TOTAL POOL</p><h2 className="text-4xl font-mono">NT$ {records.reduce((sum, r) => sum + Number(r.twdAmount), 0).toLocaleString()}</h2></div>
          <div className="bg-white rounded-[32px] p-6 shadow-sm border border-orange-50 mb-8"><input value={category} onChange={e => setCategory(e.target.value)} placeholder="消費內容..." className="w-full p-4 bg-[#F2F1EB] rounded-2xl mb-4 outline-none font-black text-black border-none shadow-inner" /><div className="grid grid-cols-2 gap-4 mb-4"><input type="number" value={amount} onChange={e => setAmount(e.target.value)} placeholder="JPY" className="w-full p-4 bg-[#F2F1EB] rounded-2xl outline-none text-[#5E9E8E] font-black shadow-inner" /><select value={expensePayerId} onChange={e => setExpensePayerId(e.target.value)} className="w-full p-4 bg-[#F2F1EB] rounded-2xl outline-none font-black shadow-inner">{allMembers.filter(m=>tripData.memberIds.includes(m.id)).map(m=><option key={m.id} value={m.id}>{m.name}</option>)}</select></div>
          <div className="flex gap-1 mb-6 overflow-x-auto no-scrollbar">{['現金', '信用卡', 'PayPay', 'Suica'].map(p => <button key={p} onClick={() => setPayMethod(p)} className={`flex-shrink-0 px-3 py-2 rounded-lg text-[10px] border font-black ${payMethod === p ? 'border-[#E9C46A] bg-[#FFF9E5] text-[#E9C46A] shadow-inner' : 'border-gray-50 text-gray-300'}`}>{p}</button>)}</div>
          <button onClick={() => { if(!category || !amount) return; const rec = { id: editingExpenseId || Date.now(), category, amount, currency: 'JPY', twdAmount: (Number(amount)*JPY_TO_TWD).toFixed(0), payMethod, payerId: expensePayerId, date: tripDates[activeDay-1] }; const n = editingExpenseId ? records.map(r => r.id === editingExpenseId ? rec : r) : [rec, ...records]; setRecords(n); syncToCloud({records: n}); setAmount(''); setCategory(''); setEditingExpenseId(null); }} className="w-full py-4 bg-[#86A760] text-white rounded-2xl font-black shadow-lg uppercase italic">{editingExpenseId ? 'Update Record' : 'Save Record'}</button></div>
          <div className="space-y-3">{records.map(r => (<div key={r.id} className="bg-white p-5 rounded-2xl flex justify-between items-center shadow-sm border border-gray-50 pr-24 relative font-black"><div className="flex items-center gap-3"><img src={getMember(r.payerId).avatar} className="w-6 h-6 rounded-full object-cover" /><div className="text-xs font-black">{r.category}<p className="text-[9px] opacity-40 font-mono italic">{getMember(r.payerId).name} · {r.payMethod}</p></div></div><div className="text-right text-[#5E9E8E] font-mono tracking-tighter">{r.amount} JPY</div><div className="absolute right-3 flex gap-4"><button onClick={() => { setEditingExpenseId(r.id); setCategory(r.category); setAmount(r.amount); setExpensePayerId(r.payerId); window.scrollTo(0,0); }} className="text-xs opacity-30">🖋️</button><button onClick={() => { if(confirm('確定刪除？')) { const n = records.filter(i => i.id !== r.id); setRecords(n); syncToCloud({records: n}); } }} className="text-xs opacity-30">🗑️</button></div></div>))}</div>
        </div>
      )}
      ...
      {activeTab === '日誌' && (
        <div className="px-4">
          <div className="bg-white rounded-[32px] p-6 shadow-sm border border-orange-50 mb-8 font-black"><h3 className="text-xs font-black mb-4 text-[#5E9E8E] uppercase tracking-widest">New Journal Entry</h3><textarea value={newJournal.content} onChange={e => setNewJournal({...newJournal, content: e.target.value})} placeholder="今天最難忘的事..." className="w-full p-4 bg-gray-50 rounded-2xl mb-4 outline-none text-sm h-32 leading-relaxed font-black shadow-inner" /><div className="flex justify-between items-center"><ImageUploader label="上傳相片" onUpload={(b64) => setNewJournal({...newJournal, image: b64})} /><button onClick={() => { if(!newJournal.content) return; const n = [{id: Date.now(), authorId: user.id, content: newJournal.content, date: tripDates[activeDay-1], image: newJournal.image}, ...journals]; setJournals(n); syncToCloud({journals: n}); setNewJournal({id: null, content: '', image: ''}); }} className="px-8 py-3 bg-[#86A760] text-white rounded-2xl font-black shadow-lg uppercase italic tracking-widest">Publish</button></div></div>
          {journals.map(j => (
            <div key={j.id} className="bg-white p-8 rounded-[32px] shadow-sm mb-6 border border-gray-50 font-black relative shadow-lg text-black"><div className="flex items-center gap-3 mb-4 text-xs text-gray-300 italic tracking-tighter"><img src={getMember(j.authorId).avatar} className="w-6 h-6 rounded-full object-cover shadow-sm border border-gray-100" /><span className="text-black font-black">{getMember(j.authorId).name} · {j.date}</span><div className="ml-auto text-black"><button onClick={() => { if(confirm('確定刪除回憶？')) { const n = journals.filter(i => i.id !== j.id); setJournals(n); syncToCloud({journals: n}); } }}>🗑️</button></div></div><p className="text-sm leading-loose whitespace-pre-wrap font-bold text-black">{j.content}</p>{j.image && <img src={j.image} className="w-full rounded-[24px] shadow-lg mt-4 border border-gray-100" />}</div>
          ))}
        </div>
      )}
      ...
      {activeTab === '成員' && (
        <div className="px-4 text-center animate-in fade-in zoom-in duration-500 font-black">
          ...
          <div className="grid grid-cols-1 gap-8 text-black font-black">{allMembers.map(m => (
      ...
      {showMemberModal && editingMember && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-md z-[100] p-8 flex items-center justify-center font-black"><div className="bg-white w-full max-w-md p-8 rounded-[48px] shadow-2xl text-black font-black"><h3 className="text-xl mb-8 italic uppercase text-center tracking-widest font-black font-black">Member Profile Setup</h3><div className="flex flex-col items-center gap-6 mb-8 font-black"><div className="relative w-32 h-32 shadow-2xl rounded-3xl overflow-hidden border-4 border-[#F9F8F3]"><img src={editingMember.avatar} className="w-full h-full object-cover" /></div><ImageUploader label="上傳相片" onUpload={(b64)=>setEditingMember({...editingMember,avatar:b64})} /></div><div className="space-y-4 mb-8 font-black"><label className="text-[10px] text-gray-400 uppercase tracking-widest font-black">Full Name</label><input value={editingMember.name} onChange={e=>setEditingMember({...editingMember,name:e.target.value})} className="w-full p-4 bg-gray-50 rounded-2xl outline-none shadow-inner border-none font-black text-xl" /><label className="text-[10px] text-gray-400 uppercase tracking-widest font-black">Login Code</label><input value={editingMember.loginCode} onChange={e=>setEditingMember({...editingMember,loginCode:e.target.value})} className="w-full p-4 bg-gray-50 rounded-2xl outline-none font-mono shadow-inner border-none font-black" /></div><div className="flex gap-4 font-black"><button onClick={()=>setShowMemberModal(false)} className="flex-1 py-4 bg-gray-100 rounded-3xl font-black uppercase font-black">Cancel</button><button onClick={()=>{const isNew=!allMembers.find(m=>m.id===editingMember.id);const up=isNew?[...allMembers,editingMember]:allMembers.map(m=>m.id===editingMember.id?editingMember:m);onUpdateMembers(up);setShowMemberModal(false);}} className="flex-1 py-4 bg-[#86A760] text-white rounded-3xl shadow-lg uppercase italic font-black">Save</button></div>{editingMember.loginCode!=='wayne' && (<button onClick={()=>{if(confirm('確定永久刪除？')){onUpdateMembers(allMembers.filter(m=>m.id!==editingMember.id));setShowMemberModal(false);}}} className="w-full mt-6 text-red-500 text-xs font-black uppercase tracking-widest font-black">✕ Permanent Delete</button>)}</div></div>
      )}

      {showPlanModal.show && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-[100] flex items-end font-black"><div className="bg-white w-full p-8 rounded-t-[48px] shadow-2xl animate-in slide-in-from-bottom text-black font-black font-black font-black"><h3 className="text-2xl mb-8 italic text-[#5E9E8E] uppercase tracking-tighter font-black">Edit Stop</h3><div className="flex gap-3 mb-6 font-black shadow-inner bg-gray-50 rounded-2xl p-2"><select className="flex-1 p-4 bg-transparent outline-none font-black text-xl" value={planForm.time.split(':')[0]} onChange={e=>setPlanForm({...planForm,time:`${e.target.value}:${planForm.time.split(':')[1]}`})}>{Array.from({length: 24}).map((_,i)=><option key={i} value={i.toString().padStart(2,'0')}>{i.toString().padStart(2,'0')} 點</option>)}</select><select className="flex-1 p-4 bg-transparent outline-none font-black text-xl" value={planForm.time.split(':')[1]} onChange={e=>setPlanForm({...planForm,time:`${planForm.time.split(':')[0]}:${e.target.value}`})}>{['00','10','20','30','40','50'].map(m=><option key={m} value={m}>{m} 分</option>)}</select></div><input placeholder="地點..." value={planForm.title} onChange={e=>setPlanForm({...planForm,title:e.target.value})} className="w-full p-5 bg-gray-50 rounded-[28px] mb-4 outline-none text-xl shadow-inner border-none font-black" /><textarea placeholder="備註..." value={planForm.desc} onChange={e=>setPlanForm({...planForm,desc:e.target.value})} className="w-full p-5 bg-gray-50 rounded-[28px] mb-8 outline-none text-sm h-32 leading-relaxed shadow-inner border-none font-black" /><div className="flex gap-4 font-black"><button onClick={()=>setShowPlanModal({show:false,type:'add'})} className="flex-1 py-4 bg-gray-100 rounded-3xl font-black uppercase font-black">Cancel</button><button onClick={()=>{if(!planForm.title)return alert("名稱必填");const dPlans=schedules[activeDay]||[];const n:Plan[]=showPlanModal.type==='add'?[...dPlans,{...planForm,id:Date.now()}]:dPlans.map(p=>p.id===showPlanModal.data?.id?{...planForm,id:p.id}:p);const up:ScheduleData={...schedules,[activeDay]:n};setSchedules(up);syncToCloud({schedules:up});setShowPlanModal({show:false,type:'add'});}} className="flex-1 py-4 bg-[#86A760] text-white rounded-3xl shadow-xl italic uppercase tracking-widest font-black font-black">Save Stop</button></div></div></div>
      )}

      {showFlightModal.show && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-[100] flex items-end font-black text-black shadow-2xl font-black"><div className="bg-white w-full p-8 rounded-t-[48px] max-h-[90vh] overflow-y-auto"><h3 className="text-2xl font-black mb-8 text-blue-500 italic uppercase">Flight Setup</h3><input placeholder="航空" value={flightForm.airline} onChange={e=>setFlightForm({...flightForm,airline:e.target.value})} className="w-full p-4 bg-gray-50 rounded-2xl mb-4 border-none shadow-inner font-black" /><input placeholder="航班號" value={flightForm.flightNo} onChange={e=>setFlightForm({...flightForm,flightNo:e.target.value})} className="w-full p-4 bg-gray-50 rounded-2xl mb-8 outline-none text-2xl font-black shadow-inner font-black" /><div className="flex gap-4 pb-12 mt-4 font-black"><button onClick={()=>setShowFlightModal({show:false,type:'add'})} className="flex-1 py-4 bg-gray-100 text-gray-400 rounded-3xl uppercase font-black">Cancel</button><button onClick={()=>{const n=[{...flightForm,id:Date.now()},...flights];setFlights(n);syncToCloud({flights:n});setShowFlightModal({show:false,type:'add'});}} className="flex-1 py-4 bg-blue-500 text-white rounded-3xl shadow-xl italic uppercase tracking-widest font-black">Confirm</button></div></div></div>
      )}
    </div>
  );
}
...
export default function AppEntry() {
  ...
  useEffect(() => {
    const m = localStorage.getItem('members_v41'); const t = localStorage.getItem('trips_v41');
  ...
  useEffect(() => { localStorage.setItem('members_v41', JSON.stringify(allMembers)); localStorage.setItem('trips_v41', JSON.stringify(allTrips)); }, [allMembers, allTrips]);
